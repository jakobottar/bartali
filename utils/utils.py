...
def agg_all_metrics(outputs, prefix="train_"):
    if len(outputs) == 0:
        return outputs
    res = {}
    ...
    for k in keys:
        ...
        if k != "epoch":
            res[prefix + k] = np.mean(all_logs)
        else:
            res[prefix + k] = all_logs[-1]
    return res
...
def viz_array_grid(
    array, rows, cols, padding=0, channels_last=False, normalize=False, **kwargs
):
    # normalization
    """
    Args:
    ...
    if channels == 1:
        canvas = np.ones(
            (H * rows + padding * (rows - 1), W * cols + padding * (cols - 1))
        )
        array = array[:, :, :, 0]
    elif channels == 3:
        canvas = np.ones(
            (H * rows + padding * (rows - 1), W * cols + padding * (cols - 1), 3)
        )
    else:
    ...
    def get_lr(self):
        ...
        for lr in self.base_lrs:
            res.append(
                np.maximum(
                    lr
                    * np.minimum(-self.last_epoch * 1.0 / self.num_epochs + 1.0, 1.0),
                    0.0,
                )
            )
        return res
