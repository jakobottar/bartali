...
def add_learner_params(parser):
    ...
    parser.add_argument(
        "--iters", default=-1, type=int, help="The number of optimizer updates"
    )
    parser.add_argument(
        "--warmup",
        default=0,
    ...
    parser.add_argument(
        "--wd", "--weight_decay", default=1e-4, type=float, dest="weight_decay"
    )
    # trainer params
    parser.add_argument(
        "--save_freq",
    ...
    parser.add_argument(
        "--eval_freq", default=10000000000000000, type=int, help="Evaluation frequency"
    )
    parser.add_argument(
        "-j", "--workers", default=4, type=int, help="The number of data loader workers"
    )
    parser.add_argument(
        "--eval_only", default=False, type=bool, help="Skips the training step if True"
    )
...
def main_worker(gpu, ngpus, args):
    ...
    while continue_training:
        ...
        for _, batch in enumerate(train_loader):
            ...
            if (
                cur_iter % args.log_freq == 0 or cur_iter >= args.iters
            ) and args.rank == 0:
                save_checkpoint(args.root, model, optimizer)
                train_logs = utils.agg_all_metrics(train_logs, prefix="train_")
